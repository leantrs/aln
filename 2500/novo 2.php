...
class PagSeguro
{
    private $email         = "leantrs@gmail.com";
    private $token_sandbox = "8D356544820741748BB4EFD4CA892942";
   // private $token_sandbox = "88616b61-de1c-4372-8398-7f4f4151a0da4e212f384ff1832407656c6b942c0ec1cf21-caf3-4903-bc23-a023d2abee9f";
...
*/
    ...
    private function generateUrl($dados, $retorno)
    {
        ...
        foreach ($retorno as $k => $v) {
            ...
            $data['itemId' . $i] = $retorno[$k]->id;
            $data['itemDescription' . $i] = $retorno[$k]->titulo;
            $data['itemAmount' . $i] =  number_format($retorno[$k]->valororiginal, 2, ".", "");
            $data['itemQuantity' . $i] = $retorno[$k]->total;
            $data['itemWeight' . $i] = '0';
    ...
    public function getStatusByCodeB($code)
    {
        $url = $this->url_transactions . $code . $this->email_token;
        $curl = curl_init($url);
	   ...
	   if ($transaction == 'Unauthorized') {
        ...
        if (isset($transaction_obj->sender->email))
            return $transaction_obj->sender->email;
        else
            return NULL;
    }
}
